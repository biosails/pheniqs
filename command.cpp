...
Prototype::Prototype(const Value& ontology) :
    ...
    if(reference != ontology.MemberEnd()) {
        ...
        } else if(reference->value.IsUint64()) {
            plural = true;
            cardinality = reference->value.GetUint64();
        } else { throw ConfigurationError("option cardinality must be an intger or *"); }
    }
...
void Action::apply_instruction_default() {
    Value default_configuration(kObjectType);
    Value::ConstMemberIterator reference = ontology.FindMember("default");
    if(reference != ontology.MemberEnd()) {
        if(!reference->value.IsNull()) {
            merge_json_value(reference->value, instruction, instruction);
        }
    }
};
void Action::apply_command_line_instruction() {
    Document overlay(kObjectType);
            ...
            if(argument->plural()) {
                switch(argument->type()) {
                    ...
                    case ParameterType::INTEGER: {
                        ...
                        if(value != NULL) { encode_key_value(key, *value, overlay, overlay); }
                        break;
                    };
                    case ParameterType::DECIMAL: {
...
void Action::expand_shell_variables(string& path) {
    if(!path.empty()) {
        string resolved;
        string variable;
        char* value(NULL);
...
bool Action::resolve_standard_stream(string& path, const IoDirection& direction) {
...
bool Action::expand_file_url(const Value& from, Value& to, Document& document, const IoDirection& direction) {
    string buffer;
    if(from.IsString()) {
...
bool Action::expand_directory_url(const Value& from, Value& to, Document& document) {
    string buffer;
    if(from.IsString()) {
        buffer.assign(from.GetString(), from.GetStringLength());
        expand_shell_variables(buffer);
...
void CommandLine::load_action_array() {
...
void CommandLine::load_sub_action(const Value& ontology) {
    string key;
...
void CommandLine::load_selected_action() {
...
ostream& CommandLine::print_version_element(ostream& o) const {
    o << name() << " version " << application_version << endl;
    return o;
};
ostream& CommandLine::print_action_element(ostream& o) const {
